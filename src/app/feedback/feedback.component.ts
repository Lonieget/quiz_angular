...
export class FeedbackComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  initCharts() {
    ...
    if (!this.statistics || !this.statistics.questionVoList) return;

    this.statistics.questionVoList.forEach((question: any, index: number) => {
      const canvasId = `chart-${question.questionId}`;
      const canvas = document.getElementById(canvasId) as HTMLCanvasElement;

      if (!canvas) return;

      const labels = question.optionsList.map((opt: any) => opt.optionName);
      const data = question.optionsList.map((opt: any) => opt.count || 0);

      const chart = new Chart(canvas, {
        type: question.type === 'M' ? 'bar' : 'doughnut',
        data: {
          labels: labels,
          datasets: [{
            label: '填答數量',
            data: data,
            backgroundColor: [
              'rgba(0, 243, 255, 0.5)',  // Neon Cyan
              'rgba(255, 0, 255, 0.5)',  // Neon Magenta
              'rgba(255, 230, 0, 0.5)',  // Neon Yellow
              'rgba(0, 255, 127, 0.5)',  // Spring Green
              'rgba(255, 62, 62, 0.5)'   // Cyber Red
            ],
            borderColor: [
